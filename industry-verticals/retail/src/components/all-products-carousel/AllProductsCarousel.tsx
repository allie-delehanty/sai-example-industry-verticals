  ...
  return (
    <div className={`component all-products-carousel py-5 ${params.styles}`} id={id}>
      <div className="container flex flex-col items-center gap-10 text-center">
        {/* Category Filter */}
        <div className="bg-background-muted flex flex-wrap justify-center rounded-sm p-1 text-base leading-8">
          {categories.map((category) => (
            <button
              className={`rounded-sm px-8 py-2 ${selectedCategory === category ? 'bg-background text-primary font-semibold shadow-sm' : 'text-foreground-light'}`}
